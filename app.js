// ─── Persistence ──────────────────────────────────────────────────────────────
const STORAGE_KEY = 'nutripro_data';
...
const S = {
  meals:   _saved?.meals   || [],
  weights: _saved?.weights || [],
  goals:   _saved?.goals   || { cal: 0, prot: 0, agua: 0 },
  water:   _saved?.water   || 0,
  profile: _saved?.profile || { peso: '', altura: '', idade: '', sexo: 'm' },
  chart:   null,
  tab:     'home',
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (S.profile) {
    if ($('peso'))   $('peso').value   = S.profile.peso   || '';
    if ($('altura')) $('altura').value = S.profile.altura || '';
    if ($('idade'))  $('idade').value  = S.profile.idade  || '';
    if ($('sexo'))   $('sexo').value   = S.profile.sexo   || 'm';
  }

  // Restore results block if goals are set
  if (S.goals.cal > 0) {
    const rb = $('results-block');
    if (rb) rb.classList.remove('hidden');
    const tmb = S.profile.sexo === 'm'
      ? 10*S.profile.peso + 6.25*S.profile.altura - 5*S.profile.idade + 5
      : 10*S.profile.peso + 6.25*S.profile.altura - 5*S.profile.idade - 161;
    setText('r-tmb',  fmt(tmb,         ' kcal'));
    setText('r-cal',  fmt(S.goals.cal,  ' kcal'));
    setText('r-prot', fmt(S.goals.prot, ' g'));
    setText('r-agua', fmt(S.goals.agua, ' ml'));
  }
